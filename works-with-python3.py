...
class PopUp_Window:
    def __init__(self, master, PopUp_Type):
        ...
        if PopUp_Type is "Set_Iteration_Rate":
            ...
            self.g = [self.n, self.MinMass, self.MaxMass, self.MinX, self.MaxX, self.MinY, self.MaxY, self.MinXvel,
                      self.MaxXvel, self.MinYvel, self.MaxYvel]
            self.b = "Make!"
            for i in self.z:
                Pos_in_List = self.z.index(i)
                Label(master, text=self.z[Pos_in_List]).grid(row=Pos_in_List, sticky=W)
    ...
    def getAttributes(self):
        if self.PopUp_Type is "Set_Iteration_Rate":
            ...
            self.master.destroy()
            for i in range(n):
                p = Particle(uniform(MinX, MaxX), (uniform(MinY, MaxY)), randint(-1, 1) * (uniform(MinXvel, MaxXvel)),
                             randint(-1, 1) * (uniform(MinYvel, MaxYvel)), uniform(MinMass, MaxMass))
...
class Particle_Data_Viewer:
    def __init__(self, p):
		...
		""" % (QUERY.Particle_Name, QUERY.birth, QUERY.death, QUERY.lifespan,
               ...
               QUERY.min_x, QUERY.max_x, QUERY.min_y, QUERY.max_y, QUERY.init_x, QUERY.init_y, QUERY.final_x,
               QUERY.final_y))
...
class Particle_Queries:
    def __init__(self, p):
        ...
        self.Vel_Graph, self.avg_vel, self.min_vel, self.max_vel, self.init_vel, self.final_vel = self.QUERY_Particle_Velocities(
            sim.DB, p)
        self.Accel_Graph, self.avg_accel, self.min_accel, self.max_accel, self.init_accel, self.final_accel = self.QUERY_Particle_Accels(
            sim.DB, p)
        self.x, self.y, self.min_x, self.max_x, self.min_y, self.max_y, self.init_x, self.init_y, self.final_x, self.final_y = self.QUERY_Particle_Positions(
            sim.DB, p)
...
class Simulation_Data_Viewer:
    def __init__(self):
        ...
        text_info = Message(master,
                            text="""Initial Iteration #: %f\n Final Iteration #: %f \n Length of Simulation: %f""" % (
                            QUERY.Init_Iter, QUERY.Final_Iter, QUERY.Sim_Length))
...
def Run():
    ...
    while sim.Paused is False:
        ...
        for i in sim.Particle_List:
            ...
            Universe.space.create_oval(i.x0, i.y0, i.x1, i.y1,
                                       fill="white")  # change color to i.color #setl in make_particle function
        Universe.space.update()
